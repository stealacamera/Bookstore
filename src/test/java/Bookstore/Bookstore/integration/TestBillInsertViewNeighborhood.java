...
public class TestBillInsertViewNeighborhood extends TestNeighborHoodBase {
	...
	static void setUp() {
		...
		books = new ArrayList<>();
		EmployeeService employeeService = new EmployeeService(new EmployeeRepository(Utils.testingUserDataDirPath, dbContext));
		createDummyData(employeeService, bookService, books);
		
		Session.setCurrentUser(employeeService.get(0));
	}
	
	static void createDummyData(EmployeeService employeeService, BookInventoryService bookService, List<BookInventoryDTO> booksList) {
		try {
